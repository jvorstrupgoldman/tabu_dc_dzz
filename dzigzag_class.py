...
def dz_sampler(dim,  # Dimension of the target
    ...
    jump_rates, jump_rates_inverse = calculate_rates(current_state, theta)

    jump_rates_max = np.maximum(jump_rates, jump_rates_inverse)

    jump_rates_sum = np.sum(jump_rates_max)
    ...
    while t < target_time:
        ...
        generator_index = np.searchsorted(jump_rates_max.cumsum(), Decimal(np.random.rand(1)[0]) * Decimal(jump_rates_sum))

        # Carry out update to the chosen generator
        if np.random.uniform(size=1) < jump_rates[generator_index]/jump_rates_max[generator_index]:
            current_state = apply_generator(current_state, generator_index, theta)
        else:
            theta[generator_index] = -1*theta[generator_index]

        jump_rates, jump_rates_inverse = update_rates(current_state, theta, jump_rates, generator_index)
